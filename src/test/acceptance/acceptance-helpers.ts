...
export function givenFullStockDataIsAvailableFor(data: {
  search: FinnhubSearchResponse;
  quote: FinnhubQuote;
  profile: Record<string, unknown>;
  metric: Record<string, unknown>;
}): void {
  server.use(
    http.get(`${BASE_URL}/search`, () => HttpResponse.json(data.search)),
    http.get(`${BASE_URL}/quote`, () => HttpResponse.json(data.quote)),
    http.get(`${BASE_URL}/stock/profile2`, () => HttpResponse.json(data.profile)),
    http.get(`${BASE_URL}/stock/metric`, () => HttpResponse.json(data.metric))
  );
}
...
export async function thenUserSees52WeekRange(expected: { low: string; high: string }): Promise<void> {
  await waitFor(() => {
